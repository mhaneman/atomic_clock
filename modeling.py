...
def gauss_func(x, x0, a, sigma, H, r):
...
def lorentzain_fit(x_data=[], y_data=[]):
    ...
    popt, pcov = curve_fit(lorentzain_func, x_data, y_data, p0=[max(y_data), mean, sigma, height])
    ym = lorentzain_func(x_data, popt[0], popt[1], popt[2], popt[3], popt[4])
    return ym
...
def gauss_fit(x_data=[], y_data=[]):
    ...
    ym = gauss_func(x_data, popt[0], popt[1], popt[2], popt[3])
    return ym